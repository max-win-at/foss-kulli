...
function createBoard(opts = {}) {
  ...
  const vmDom = {
    viewportWidth: 1024,
    viewportHeight: 768,
    focusEditorWithContent: vi.fn(() => true),
    getEditorContent: vi.fn(() => ""),
    moveCursorToEnd: vi.fn(),
    analyzeClickTarget: vi.fn(() => ({
      isOnStickyNote: false,
      isOnEditingStickyNote: false,
      isOnFab: false,
      isOnPopup: false,
      isOnEmptyStateContainer: false,
      isOnSelectedNoteMenu: false,
      stickyNoteElement: null,
    })),
    findClosestStickyNote: vi.fn(() => null),
    findClosestFab: vi.fn(() => null),
    findClosestPopup: vi.fn(() => null),
    findClosestEmptyStateContainer: vi.fn(() => null),
    findClosestSelectedNoteMenu: vi.fn(() => null),
    isElementEditing: vi.fn(() => false),
    getNoteEditor: vi.fn(() => null),
  };
  ...
  return {
    ...
    srvLocalStorage,
    vmDom,
    noteFactory: opts.noteFactory || noteFactory,
    config,
  };
}
...
describe("VmWhiteBoard", () => {
  describe("matchesSearch", () => {
    ...
    it("returns false when note is null", () => {
      ...
      // Note: matchesSearch now checks deleted flag first, so null note throws
      // This test verifies the function handles null gracefully
      expect(() => board.matchesSearch(null)).toThrow();
    });
  ...
  describe("getNextNotePosition", () => {
    ...
    it("places next note to the right of the last note", () => {
      const { board } = createBoard();
      board.notes = [new MdlStickyNote("n1", "Hi", 240, 24)];
    ...
    it("wraps to the next row when viewport overflows", () => {
      ...
      board.notes = [new MdlStickyNote("n1", "Hi", 300, 24)];
    ...
    it("uses noteGap for wrapped rows below the stack area", () => {
      ...
      board.notes = [new MdlStickyNote("n1", "Hi", 300, 300)];
    ...
    it("includes the editing note in position calculations", () => {
      ...
      const { board } = createBoard();
      const editingNote = new MdlStickyNote("editing", "Draft", 240, 24);
    ...
    it("handles multiple notes filling a row", () => {
      ...
      board.notes = [
        new MdlStickyNote("n1", "A", 240, 24),
        new MdlStickyNote("n2", "B", 440, 24),
      ];
  ...
  describe("rearrangeNotes", () => {
    ...
    it("repositions notes to fit the viewport", () => {
      ...
      board.viewportWidth = 500;
      board.notes = [
        new MdlStickyNote("n1", "A", 0, 0),
        new MdlStickyNote("n2", "B", 0, 0),
        new MdlStickyNote("n3", "C", 0, 0),
      ];
    ...
    it("calls save() after rearranging", () => {
      const { board, srvLocalStorage } = createBoard();
      board.notes = [new MdlStickyNote("n1", "A", 0, 0)];
    ...
    it("includes the editing note in the layout", () => {
      ...
      const note1 = new MdlStickyNote("n1", "A", 0, 0);
      const editingNote = new MdlStickyNote("editing", "Draft", 0, 0);
  ...
  describe("applySearchFilter / restorePositions", () => {
    it("restores positions when query is cleared", () => {
      const { board } = createBoard();
      board.notes = [
        new MdlStickyNote("n1", "Hello", 100, 200),
        new MdlStickyNote("n2", "World", 300, 400),
      ];
    ...
    it("saves original positions before first filter", () => {
      const { board } = createBoard();
      board.notes = [new MdlStickyNote("n1", "Match", 111, 222)];
    ...
    it("does not overwrite original positions on subsequent filters", () => {
      const { board } = createBoard();
      board.notes = [new MdlStickyNote("n1", "Hello World", 111, 222)];
    ...
    it("filters notes by search match and sorts by createdAt descending", () => {
      ...
      const older = new MdlStickyNote("n1", "Match older", 0, 0, "2024-01-01");
      const newer = new MdlStickyNote("n2", "Match newer", 0, 0, "2025-06-01");
      const noMatch = new MdlStickyNote("n3", "No result", 0, 0);
    ...
    it("restorePositions no-ops when no positions are saved", () => {
      const { board } = createBoard();
      board.notes = [new MdlStickyNote("n1", "Hi", 50, 60)];
  ...
  describe("clearSearch", () => {
    ...
    it("restores original positions", () => {
      const { board } = createBoard();
      board.notes = [new MdlStickyNote("n1", "Hi", 77, 88)];
  ...
  describe("confirmEditing", () => {
    it("keeps editing note in notes[] if text is non-empty", () => {
      ...
      const { board } = createBoard();
      const note = new MdlStickyNote("n1", "Hello", 10, 20);
      board.notes = [note];
    ...
    it("does not add editing note if text is empty or whitespace", () => {
      const { board } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "   ", 10, 20);
    ...
    it("calls save() after confirming", () => {
      const { board, srvLocalStorage } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "Hello", 10, 20);
  ...
  describe("cancelEditing", () => {
    it("keeps editing note in notes[] if text is non-empty", () => {
      // With the new approach, notes stay in array during editing
      const { board } = createBoard();
      const note = new MdlStickyNote("n1", "Draft", 10, 20);
      board.notes = [note];
      board.editingNote = note;
  ...
  describe("deleteEditingNote", () => {
    it("moves note to trash via storage service", () => {
      vi.useFakeTimers();
      const { board, srvLocalStorage } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "Delete me", 10, 20);

      board.deleteEditingNote();
      vi.advanceTimersByTime(300);

      expect(srvLocalStorage.moveToTrash).toHaveBeenCalledWith(
        board.editingNote || expect.anything(),
      );
      vi.useRealTimers();
    });
  ...
  describe("editNote", () => {
    it("keeps the target note in notes[] (array order stability)", () => {
      ...
      const { board } = createBoard();
      const note = new MdlStickyNote("n1", "Edit me", 10, 20);
      board.notes = [note];
    ...
    it("clears previous editing state when switching to new note", () => {
      ...
      const { board } = createBoard();
      const existing = new MdlStickyNote("n1", "Existing", 10, 20);
      const newNote = new MdlStickyNote("n2", "New edit", 30, 40);
    ...
    it("sets editor content to the correct note text when switching between notes", async () => {
      ...
      // Mock vmDom.focusEditorWithContent to capture the text
      let capturedTextContent = null;
      board._vmDom.focusEditorWithContent = vi.fn((text) => {
        capturedTextContent = text;
        return true;
      });

      // Setup: two notes exist
      const note1 = new MdlStickyNote("n1", "First note text", 10, 20);
      const note2 = new MdlStickyNote("n2", "Second note text", 30, 40);
    ...
    it("preserves correct note reference in setTimeout when notes array is modified", async () => {
      ...
      const oldNote = new MdlStickyNote("old", "Old text", 0, 0);
      const newNote = new MdlStickyNote("new", "New text", 10, 10);
    ...
    it("handles deferred setTimeout execution correctly (stale closure regression test)", async () => {
      ...
      // Setup: two notes
      const note1 = new MdlStickyNote("n1", "Note One", 10, 20);
      const note2 = new MdlStickyNote("n2", "Note Two", 30, 40);
      board.notes = [note1, note2];
    ...
    it("guards against null editingNote in deferred $nextTick", () => {
      ...
      board.$refs = {
        noteEditor: {
          set textContent(value) {
            capturedTextContent = value;
          },
          get textContent() {
            return capturedTextContent;
          },
          focus: vi.fn(),
        },
      };
      ...
      const note = new MdlStickyNote("n1", "Test", 10, 20);
      board.notes = [note];
  ...
  describe("editNoteById", () => {
    it("finds note by ID and calls editNote", () => {
      const { board } = createBoard();
      const note1 = new MdlStickyNote("n1", "First", 10, 20);
      const note2 = new MdlStickyNote("n2", "Second", 30, 40);
      board.notes = [note1, note2];

      const editNoteSpy = vi
        .spyOn(board, "editNote")
        .mockImplementation(() => {});
    ...
    it("does nothing if note ID is not found", () => {
      const { board } = createBoard();
      board.notes = [new MdlStickyNote("n1", "Test", 10, 20)];
  ...
  describe("createNewNote", () => {
    ...
    it("does nothing if already editing", () => {
      const { board } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "Draft", 0, 0);
  ...
  describe("updateEditingText", () => {
    it("updates the editing note text", () => {
      const { board } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "Old", 0, 0);
  ...
  describe("onEditingKeyDown", () => {
    ...
    it("Ctrl+Enter confirms editing", () => {
      const { board } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "Text", 0, 0);
  ...
  describe("onWhiteBoardClick", () => {
    ...
    it("does nothing when clicking on a sticky note", () => {
      ...
      // Mock analyzeClickTarget on the board's internal vmDom reference
      board._vmDom.analyzeClickTarget = vi.fn(() => ({
        isOnStickyNote: true,
        isOnEditingStickyNote: false,
        isOnFab: false,
        isOnPopup: false,
  ...
  describe("getMenuPosition", () => {
    ...
    it("returns correct CSS positioning when editing", () => {
      const { board } = createBoard();
      board.editingNote = new MdlStickyNote("n1", "Hi", 100, 200);
  ...
  describe("array order stability (regression tests)", () => {
    it("notes array order unchanged after editing existing note", () => {
      ...
      const note1 = new MdlStickyNote("n1", "First", 100, 20);
      const note2 = new MdlStickyNote("n2", "Second", 200, 20);
      const note3 = new MdlStickyNote("n3", "Third", 300, 20);
    ...
    it("note positions unchanged after edit cycle", () => {
      ...
      const originalPositions = board.notes.map((n) => ({
        id: n.id,
        x: n.x,
        y: n.y,
      }));
      ...
      const finalPositions = board.notes.map((n) => ({
        id: n.id,
        x: n.x,
        y: n.y,
      }));
    ...
    it("cancelling new empty note soft-deletes it (stays in array with deleted flag)", () => {
      const { board } = createBoard();
      board.notes = [];
      ...
      // Soft-delete: note stays in array but is marked deleted
      expect(board.notes.length).toBe(1);
      expect(board.notes[0].deleted).toBe(true);
      // visibleNotes should be empty
      expect(board.visibleNotes.length).toBe(0);
    });

    it("cancelling existing note with content keeps it in array", () => {
      const { board } = createBoard();

      const note1 = new MdlStickyNote("n1", "Has content", 100, 20);
      board.notes = [note1];
